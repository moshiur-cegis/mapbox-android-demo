...
public class ExpandingCircleRadiusAnimationActivity extends AppCompatActivity {
  ...
  private static final float CIRCLE_LAYER_OPACITY = .7F;
  private static final boolean fadeCircleExpansion = true;
            ...
            .withImage(POWER_PLANT_SYMBOL_ICON_ID, BitmapFactory.decodeResource(
                ...
                public void onStyleLoaded(@NonNull Style style) {
                    ...
                    circleLayer.withProperties(
                        circleOpacity(CIRCLE_LAYER_OPACITY),
   ...
   */
  private void animateCircleRadius() {
    ...
    expandingCircleAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
      ...
      public void onAnimationUpdate(ValueAnimator valueAnimator) {
        ...
        // Update the circles' radii with the new value
        circleLayer.setProperties(circleRadius(newPulsingRadiusValue));

        // Adjust the CircleLayer opacity as the radius increases
        if (fadeCircleExpansion) {
          double newPulsingOpacityValue = 1 - ((newPulsingRadiusValue / 55) * 2.5);
          circleLayer.setProperties(circleOpacity((float) newPulsingOpacityValue));
        }
      }
    });
